...
GPU1="${GPU1:-1}"
DATA="${DATA:-data}"
OUTPUT="${OUTPUT:-runs_curvature_probe}"
mkdir -p "$OUTPUT/logs"
...
COMMON=(
  ...
  --curvature-damping 0.1 --mobility-minimum 0.05 --mobility-maximum 3
  --permutation-seed 0 --data "$DATA" --output "$OUTPUT" --seed 0
)

CUDA_VISIBLE_DEVICES="$GPU0" "$PYTHON" -u -m gwd_cl.train "${COMMON[@]}" \
  --curvature-strength 0.5 --tag curvature_half \
  >"$OUTPUT/logs/curvature_half.log" 2>&1 &
PID0=$!
CUDA_VISIBLE_DEVICES="$GPU1" "$PYTHON" -u -m gwd_cl.train "${COMMON[@]}" \
  --curvature-strength 1.0 --tag curvature_full \
  >"$OUTPUT/logs/curvature_full.log" 2>&1 &
PID1=$!
...
if [[ "$STATUS0" -ne 0 || "$STATUS1" -ne 0 ]]; then
  echo "A curvature probe failed. Inspect $OUTPUT/logs/curvature_*.log"
  exit 1
fi

echo "curvature strength 0.5:"
tail -n 2 "$OUTPUT/logs/curvature_half.log" | head -n 1
echo "curvature strength 1.0:"
tail -n 2 "$OUTPUT/logs/curvature_full.log" | head -n 1
